    ...
    def index(self, **kw):
        ...
        for lpn_contact in lpn_res_partner:
            ...
            if truecontactverified:
                ...
                company = lpn_contact.parent_id  # Utilisez "parent_id" si le champ est nommé ainsi, sinon utilisez "company_id"
                if company:
...
#        sales_orders = http.request.env['sale.order'].search([])
        ...
        for lpn_contact in lpn_res_partner:
            ...
            # Vérifiez et concaténez la valeur de 'company_name'
            company = lpn_contact.parent_id  # Utilisez "parent_id" si le champ est nommé ainsi, sinon utilisez "company_id"
            if company:
                output +=      str(f"{company.name}") + ';'
            else:
