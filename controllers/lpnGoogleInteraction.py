...
def Recherche_main(lpn_query):
    ...
    for contact in GLOBAL_CONNECTION_LIST:
        ...
        #GLOBAL_LOGGER.logger.info(f"Name of contact : {display_name}" + "\n")
        contact_indice = display_name.lower().find(lpn_query.lower())
        if contact_indice != -1:
        #    GLOBAL_LOGGER.logger.info(f"La sous-chaîne '{lpn_query}' a été trouvée à l'indice {contact_indice}.")
...
def update_contact(Nouveau_nom,email_add,tel_num,mob,compagnie,Job):
    ...
    try:
        ...
        # Creer le contact 
        #print("nom à updater: " + Nouveau_nom) 
